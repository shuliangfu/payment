...
- **Payment adapters** (`src/adapters/`): Stripe, PayPal, Alipay, WeChat, Apple Pay, Google Pay, UnionPay, Web3 (crypto); unified adapter interface; create payment, query, refund.
...
- **`@dreamer/logger` dependency**: Added `@dreamer/logger` (type-only, used in `src/adapters/types.ts`) to `deno.json` imports and `package.json` dependencies.
- **i18n in adapters**: Replaced hardcoded Chinese strings in `src/adapters` with `$tr()` calls for Node-compatible localization via `@dreamer/runtime-adapter`.
...
- **Docs**: Restructured into `docs/en-US` and `docs/zh-CN`; standardized `LICENSE`.
