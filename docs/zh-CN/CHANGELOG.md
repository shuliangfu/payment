...
- **`@dreamer/logger` 依赖**：将 `@dreamer/logger`（type-only，用于 `src/adapters/types.ts`）加入 `deno.json` imports 与 `package.json` dependencies 以保证正确性。
...
- **正式版发布**：首个正式版本，API 稳定。
- **支付适配器**（`src/adapters/`）：Stripe、PayPal、Web3（加密货币）；统一适配器接口；创建支付、查询、退款。
- **国际与国内**：多支付方式、多币种支持。
