...
| 版本 | 1.0.0 |
| 单元测试数 | 84 |
| 集成测试数 | 5（web3.test.ts，独立任务） |
| 通过 | 84（单元） |
| 失败 | 0 |
| 跳过 | 0 |
| 通过率 | 100% |
| 依赖 | crypto、i18n、logger（type-only）、runtime-adapter |
...
| 运行时 | 测试数 | 状态 |
|--------|--------|------|
| Deno 2.9+ | 89（84 单元 + 5 生命周期钩子） | ✅ 通过 |
| Bun 1.3+ | 84 | ✅ 通过 |
| Node.js 22+ | 84 | ✅ 通过 |

> Deno 在 84 个单元测试之上额外统计 5 个生命周期钩子。Bun 与 Node 报告 84 个单元测试。`web3.test.ts` 的 5 个集成测试需本地 Anvil 节点 `127.0.0.1:8545`，已拆为独立 `test:integration` 任务，排除出 CI。
...
|------|--------|------|------|
| mod.test.ts | 15 | 单元 | ✅ |
| currency.test.ts | 20 | 单元 | ✅ |
| reconciliation.test.ts | 15 | 单元 | ✅ |
| crypto.test.ts | 22 | 单元 | ✅ |
| subscription.test.ts | 12 | 单元 | ✅ |
| **单元小计** | **84** | | ✅ |
| web3.test.ts | 5 | 集成 | ⚙️（需 Anvil） |
...
### Web3 支付测试（集成测试，需 Anvil）

> 以下 Web3 用例属于集成测试（`web3.test.ts`），需本地 Anvil 节点 `127.0.0.1:8545`，通过 `test:integration` 任务运行，不纳入 CI。
...
- **测试框架**: @dreamer/test
- **CI**: 9 任务矩阵（Deno 2.9 / Bun 1.3 / Node 22 × Linux/macOS/Windows）
...
全部 84 个单元测试在 Deno 2.9+、Bun 1.3+ 和 Node.js 22+ 三运行时下均通过（0 失败，100% 通过率）。`web3.test.ts` 的 5 个集成测试已拆为独立的 `test:integration` 任务，排除出 CI。本包在三运行时下均已达到生产就绪状态。
